...
```r format(mean(steps.by.day$steps,na.rm=TRUE),digits=2,scientific=FALSE)``` and the median is ```r format(median(steps.by.day$steps, na.rm= TRUE),digits=2)```. 
...
The mean is ```r format(mean(imp.steps.per.day$steps),scientific=FALSE,digits=6) ``` and the median is ```r format(median(imp.steps.per.day$steps),scientific=FALSE,digits=6)```   

The mean of steps  of the original data set is
```r format(mean(steps.by.day$steps),,digits=6,scientific=FALSE)``` ,
the mean of the imputed data set is
```r format(median(imp.steps.per.day$steps),digits=3,scientific=FALSE)```,
which is the same...  
